...
export default {
    getClockUser(userId, start, end) {
        return Api().get(
                `/clocks/${userId}?${start ? `start=${start}` : ""}${
        end ? `&end=${end}` : ""
      }`
    );
  },

  getDailyClocksUser(userId) {
    let day = new Date();
    ...
    return Api().get(
      `/clocks/${userId}?start=${fd}&end=${ld}`
    );
  },

  getWeeklyClocksUser(userId) {
    const startOfWeek = () => {
      let d = new Date();
      var day = d.getDay(),
    ...
    return Api().get(`/clocks/${userId}?start=${fd}&end=${ld}`);
  },

  getMonthlyClocksUser(userId) {
